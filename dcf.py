...
def _normalize_ticker_records(data):
    """Return iterable of ticker records regardless of JSON shape (list or dict)."""
...
def cik_lookup(ticker: str, ua: str) -> str:
    """Return zero-padded 10-digit CIK for a given ticker using the SEC mapping.
    Handles both list and dict JSON formats from /files/company_tickers.json.
    """
...
def _extract_annual_series(facts: dict, tag: str, unit_preference=("USD", "shares"), limit: int = 10) -> pd.DataFrame:
    """Extract latest annual values (FY) for a us-gaap tag from companyfacts."""
    try:
...
def build_financials(facts: dict) -> pd.DataFrame:
    """Build compact annual table with CFO, CapEx, Revenue, Net Income, Cash, Debt, Shares, and FCF."""
...
def build_statements(facts: dict, limit: int = 10):
    """Return dict of DataFrames for Income Statement, Balance Sheet, and Cash Flow Statement (annual FY)."""
...
def dcf_two_stage(last_fcf: float, g_years: int, growth: float, wacc: float, terminal_growth: float):
    """Two-stage DCF with constant growth then Gordon terminal."""
...
# ==============================================
# App UI with persistent data (no emojis, Q&A at bottom of every tab)
# ==============================================
...
if load:
    ...
    # Ticker changed: require load to fetch new company
...
if not store or store.get("ticker") != ticker:
    st.stop()  # Stop rendering until data is loaded for this ticker

# Recompute DCF every run using current sliders, but reuse fetched financials
fins = store["fins"]
stmts = store["stmts"]
# choose last available
last_row = fins.dropna(subset=["FCF"]).iloc[-1]
last_fcf = float(last_row["FCF"])  # USD
...
with tab_export:
    ...
    with pd.ExcelWriter(buf, engine="xlsxwriter") as writer:
        ...
        store["fins"].to_excel(writer, index=False, sheet_name="FCF_Build")
    ...
    st.download_button("All Sheets (Excel)", data=buf.getvalue(), file_name=f"{store['ticker']}_dcf_export.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
